import { ChangedFile, FeatureBlock, RiskLevel, ValidationStatus } from "../core/architectureModel";
import { graphViewBox } from "../graph/svgGraphLayout";
import {
  DashboardMode,
  DashboardState,
  DiffChangeRow,
...
function renderModeTab(state: DashboardState, mode: DashboardMode): string {
...
function renderMode(state: DashboardState): string {
  switch (state.mode) {
...
function renderLiveChangesMode(state: DashboardState): string {
  return `
...
function renderWholeArchitectureMode(state: DashboardState): string {
  return `
...
function renderFeatureFocusMode(state: DashboardState): string {
...
function renderDiffSinceBaselineMode(state: DashboardState): string {
  return `
    <section class="mode-grid">
...
function renderChangedFilesTable(files: ChangedFile[]): string {
  return `
...
function renderValidationStatus(validations: ValidationStatus[]): string {
  return `
...
function renderWholeArchitectureSideNav(state: DashboardState): string {
  return `
...
function renderFeatureSideNav(state: DashboardState, selected: FeatureBlock): string {
...
function renderDiffSideNav(state: DashboardState): string {
  return `
    <aside class="panel side-nav">
      <div class="side-group">
...
function renderMetricGrid(cards: MetricCard[], className: string): string {
  return `
    <div class="${className}">
      ${cards.map((card) => `
...
function renderHealthCards(cards: HealthCard[]): string {
  return `
...
function renderExternalDependencies(state: DashboardState): string {
  return `
...
function renderTestList(files: ChangedFile[]): string {
...
function renderTopChanges(changes: DiffChangeRow[]): string {
  return `
...
function renderTimeline(state: DashboardState): string {
  ...
  const line = (key: "modules" | "dependencies" | "tests", color: string) => {
...
function renderGraph(graph: GraphModel, controlLabel: string, controls: boolean): string {
  const nodeLookup = new Map(graph.nodes.map((node) => [node.id, node]));
  return `
    <div class="graph-wrap">
      ${controls ? renderGraphControls(controlLabel) : ""}
      <svg viewBox="${graphViewBox(graph)}" role="img" aria-label="Architecture graph">
        <defs>
...
function renderGraphControls(label: string): string {
  return `
...
function renderGraphEdge(fromId: string, toId: string, kind: string, nodeLookup: Map<string, GraphNode>): string {
...
function centerOf(node: GraphNode): { x: number; y: number } {
...
function statusShort(status: ChangedFile["status"]): string {
  switch (status) {
    case "modified":
      return "M";
    case "added":
...
function featureLabel(featureId: string | undefined): string {
  switch (featureId) {
    case "config-system":
...
function featureTone(featureId: string): "high" | "medium" | "low" | "info" {
  switch (featureId) {
    case "safety-layer":
    case "robot-io":
      return "medium";
...
function externalDetail(featureId: string): string {
  switch (featureId) {
    case "gui-layer":
...
function mockModuleCount(featureId: string): number {
  switch (featureId) {
    case "gui-layer":
      return 12;
    case "motion-planning":
