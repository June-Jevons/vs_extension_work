...
assert.ok(gitProviderSource.includes("change.resourceUri ?? change.uri"), "live Git provider should support both VS Code Git API change URI shapes");
assert.ok(gitProviderSource.includes("change.type ?? change.status"), "live Git provider should support both VS Code Git API change status shapes");
