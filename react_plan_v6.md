...
This v6 plan supersedes `react_plan_v1.md`, `react_plan_v2.md`, `react_plan_v3.md`, `react_plan_v4.md`, and `react_plan_v5.md` for implementation.

Use this file only unless the user explicitly asks otherwise.

---

## 1. What Was Missing From v5

v5 was strong, but these items were still needed for a 10/10 execution plan:

1. **Toolchain reproducibility**
   - Pin/declare Node version.
   - Add `.nvmrc` and optionally `.node-version`.
   - Add `package.json` `engines`.
   - Commit `package-lock.json` after dependency changes.
   - Use `npm ci` for repeatable validation after the lockfile exists.

2. **Mandatory Ubuntu doctor**
   - `doctor:ubuntu` is mandatory, not optional.
   - It verifies and reports only.
   - It must not install, rewrite, or silently fix anything.

3. **F5 and `.vscode` validation is testable**
   - Add tests that parse `.vscode/launch.json` and `.vscode/tasks.json`.
   - Tests fail if Windows/WSL/PowerShell entries remain.
   - F5 uses `npm: compile` only.
   - F5 does not run hidden install fallback.

4. **Ubuntu docs are first-class**
   - README active workflow becomes Ubuntu Native.
   - Add or update `docs/ubuntu_setup.md` if docs are split.
   - Windows/WSL instructions are removed from active workflow or clearly marked obsolete.

5. **CI / repeatable validation path**
   - Add Ubuntu CI if practical.
   - CI should run compile, unit tests, visual render/test, package, and VSIX verification.
   - VS Code integration may run under explicit Xvfb if stable.

6. **Fixture workspace for tests**
   - Automated tests must not require `/home/jevons/ABB_ROS2`.
   - Add a small Linux-native Python fixture workspace under `test/fixtures/`.
   - Real ABB_ROS2 remains manual/smoke validation target.

7. **Cache schema/versioning**
   - File cache and workspace index must have schema versions.
   - Incompatible/corrupt cache causes explicit invalidation/error state, not fallback.

8. **Performance budgets and regression reporting**
   - Define conservative measurable timing checks.
   - Always report timings.
   - Do not claim performance success without numbers.

9. **Security and packaging checks**
   - CSP must be inspected/tested.
   - VSIX contents must be verified by script.
   - Packaged extension must not need source files, Vite dev server, or test artifacts.

10. **Checkpoint execution guidance**
   - Do not complete the whole migration in one unreviewed pass.
   - Pass 1 must be environment + `.vscode` + docs + path tests before React migration.

---

## 2. Native Ubuntu 22.04 Environment Policy
...
## 3. First-Time Ubuntu Setup Requirements

Because this is a new native Ubuntu environment, verify prerequisites before implementation.

### 3.1 Required system tools

Check and install if missing. These setup commands are explicit and must not be hidden inside F5/tasks:
...
If `sudo` is unavailable or packages cannot be installed, stop and report the missing dependencies. Do not reduce validation scope silently.

### 3.2 Required development tools
...
- Use `npm ci` for repeatable validation once `package-lock.json` exists.
- Do not run hidden `npm install` or `npm ci` from F5 or VS Code tasks.

### 3.4 Playwright/browser dependencies
...
## 4. Critical Policy: No Fallback Features
...
## 5. Existing Ubuntu Migration Issues To Fix
...
### 5.4 Optional `.vscode/settings.json`
...
### 5.5 Path kind and storage tests
...
### 8.1 Watcher/cache vs Git split
...
### 8.5 Git provider without fallback
...
### 8.6 Deterministic native Linux scanner backend
...
### 8.7 Performance diagnostics and budgets
...
## 11. Legacy and Fallback Removal Rules
...
## 12. VSIX and Packaging Requirements
...
Stop and report after each pass if validation fails.
