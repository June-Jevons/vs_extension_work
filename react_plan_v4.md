...
This v4 plan supersedes `react_plan_v1.md`, `react_plan_v2.md`, and `react_plan_v3.md` for implementation.

Use this file only unless the user explicitly asks otherwise.

Major v4 environment change:

```text
Implementation and validation environment is now native Ubuntu 22.04.
It is not WSL.
It is not Windows Native VS Code opening a WSL UNC path.
Do not use Windows paths, PowerShell, code.cmd, \\wsl.localhost, or \\wsl$ instructions.
```
...
6. Validate everything on native Ubuntu 22.04.
...
## 1. v4 Environment Policy: Native Ubuntu 22.04 Only

All implementation and validation must run on native Ubuntu 22.04.
...
Use the actual local clone path if different from `/home/jevons/vs_extension_work`.

Use Linux-native paths for target workspaces, for example:
...
VS Code Extension Development Host example for native Ubuntu:
...
If the machine is headless or display capture fails, report the display/session issue explicitly. Do not replace live validation with mock-only evidence.

---

## 2. v4 Critical Policy: No Fallback Features
...
1. **Native Ubuntu 22.04 workflow only**
   - Do not implement or validate from WSL.
   - Do not use Windows Native VS Code commands.
   - Use Bash/Linux shell commands.
...
Current provider may use VS Code Git API and then Git CLI. v4 policy requires no automatic fallback.
...
### 7.6 Deterministic native Linux scanner backend, no fallback
...
- Prefer npm tasks over custom shell command strings.
- Use Bash-compatible commands only.
- Do not use PowerShell.
- Do not use Windows paths.
- F5 must run compile reliably on native Ubuntu VS Code.
...
After packaging, install into isolated extensions directory and launch against target workspace using native Ubuntu VS Code.
...
- Replace scanner fallback with deterministic native Linux scanner backend selection.
...
- Native Ubuntu path uses deterministic `vscodeFindFiles` backend by default.
...
- Installed extension loads React dashboard on native Ubuntu.
- No inspected target workspace files are written.
...
- Native Linux paths select `vscodeFindFiles` by default.
...
- Implementation and validation were performed on native Ubuntu 22.04.
- No Windows, PowerShell, WSL, WSL UNC, or Remote-WSL workflow remains in the active instructions/tests.
...
Run from native Ubuntu 22.04 Bash shell in the extension repository.
...
Known limitations
- Anything still requiring manual VS Code UI inspection on Ubuntu
- Any display/session limitations
...
- Do not include Windows-specific or WSL-specific validation instructions in final docs unless they are clearly marked as obsolete historical notes.
